...
{
    selected_algorithm = par("selected_algorithm");

    if (stage == 0) {

        // get parameters
        maximumContentStoreSize = par("maximumContentStoreSize");
        //CRPAlgorithms *crp  = new CRPAlgorithms();
...
//            << contentObjMsg->getDataName() << endl;
    ...
    CRPAlgorithms *crpalgorithm = new CRPAlgorithms();
    crpalgorithm->setValues(cs,currentCSSize,selected_algorithm);
    csEntry = new CSEntry;
    long removedBytes = 0;
    std::map<CSEntry*, int> accessCounts;
    crpalgorithm->crphandle(contentObjMsg,csEntry, maximumContentStoreSize,&removedBytes, accessCounts);
    cs = crpalgorithm->getCS();
    currentCSSize = crpalgorithm->getcurrentCSSize();
    // generate stats
         if (removedBytes > 0) {
